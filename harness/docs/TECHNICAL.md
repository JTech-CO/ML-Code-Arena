...
```
/judge/                        제출별 작업 디렉터리 (읽기 전용 마운트)
  solution.py                  사용자 제출 코드
  spec.json                    채점 명세 (제한 규칙 + 허용 오차 + 케이스 수)
  cases/
    case_00.json               입력 구조
    case_00.npz                입력에 실린 배열
    expect_00.json             기대 반환값 구조
    expect_00.npz              기대 반환값에 실린 배열
    ...

/opt/mlca/runner/              채점 러너 (읽기 전용 마운트, 별도 볼륨)
  runner.py  spec.py  ast_check.py  compare.py  codec.py
```

**러너를 `/judge` 가 아니라 별도 볼륨에 두는 이유** (M1 에서 확정): 러너는 제출과 수명이 다르다. 제출별 작업 디렉터리에 러너를 복사하면 제출마다 같은 파일을 복제하게 되고, 워커가 그 디렉터리를 만드는 과정에서 러너가 변조될 경로가 생긴다. 두 볼륨으로 나누면 러너는 저장소의 원본 한 부만 존재한다.

테스트케이스 직렬화는 **numpy `.npz` + JSON 쌍**을 쓴다(ADR-0003). 구조는 JSON 의 태그 트리로, 배열 본체는 `.npz` 로 담고 JSON 이 키로 참조한다. 관용 키(`{"__nd__": ...}`) 대신 모든 값을 태그로 감싸는 이유는 사용자 dict 와 메타데이터의 키 충돌을 원리적으로 없애기 위해서다. 로더는 태그 화이트리스트와 dtype 화이트리스트로 두 번 검증한다.

역직렬화 시 임의 코드 실행이 가능한 포맷은 신뢰 경계를 넘는 데이터에 부적합하다(INV-7). 지금은 우리가 만든 파일만 컨테이너에 들어가지만, Phase 2 에서 데이터셋 스토리지가 생기면 그 경계가 넓어진다.
...
```jsonc
{
  ...
  "cases": [
    ...
    { "index": 1, "verdict": "WA", "runtime_ms": 9,
      "detail": { "reason": "shape_mismatch",
                  "expected_shape": [3, 3], "actual_shape": [3] } }
  ],
  "total_runtime_ms": 21,
  "peak_memory_mb": 48,
  "error": null,

  // 케이스에 매달 수 없는 판정의 상세. M1 에서 추가 확정.
  "detail": { "violations": [ { "rule": "import_not_allowed",
                                "message": "...", "line": 3 } ] }
}
```

**최상위 `detail` 이 필요한 이유** (M1 에서 확정): `CE` 와 `FBD` 는 케이스를 하나도 실행하지 않으므로 케이스 배열에 상세를 실을 자리가 없다. `FBD` 가 어떤 규칙을 어겼는지 말하지 못하면 §4.3 의 요구를 지킬 수 없고, `CE` 가 몇 행인지 말하지 못하면 사용자가 고칠 수 없다. `TLE`·`MLE` 는 여기에 걸린 상한을 싣는다.

`total_runtime_ms` 는 **엔트리포인트 호출 시간의 합**이며 컨테이너 기동 시간을 포함하지 않는다. 콜드 스타트(200~400ms)가 측정에 섞이면 `TLE` 오탐이 난다(런북 22번).

사용자에게는 **첫 실패 케이스의 인덱스와 shape 정보까지만** 노출한다. 기대값 자체는 절대 노출하지 않는다(INV-5). 러너는 첫 실패에서 멈추므로 `cases` 배열은 거기까지만 채워진다.
